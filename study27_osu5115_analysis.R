...
# Define nominal sample times
  samp.times <- c(0, 0.5, 1, 2, 3, 4, 6, 8, 24)
...
# Use ddply to mean the normalised DV for each of the bins
  meanpool <- ddply(clindata, .(TADNOM), function(df) {
    out <- data.frame(MEAN = mean(df$DV))
  })
...
# Visualisation of mean pooled data
  ...
  p1 <- p1 + geom_point(aes(x = TIME, y = DV), data = clindata,
...
# Output is list due to variable lengths of time vectors
  interv.res <- llply(6:9, function(i) {
    metric <- llply(1:1000, function(j) {
      res <- optim.interv.dtmax(opt.sumexp, seq(0, 24, length.out = i))
      out.times <- res$times
      auc024 <- auc.interv.sumexp(out.times, opt.sumexp)
      pred <- pred.sumexp(opt.sumexp, out.times)
      list(
        times = rbind(out.times),
        auc024 = auc024,
        auc0inf = auc024 + auc.interv.lam(opt.sumexp, out.times),
        cmax = max(pred),
        tmax = out.times[which(pred == max(pred))]
      )
    })
    out.metrics <- cbind(
      colwise(mean)(ldply(metric, function(x) {x$auc024})),
      colwise(CI90lo)(ldply(metric, function(x) {x$auc024})),
      colwise(CI90hi)(ldply(metric, function(x) {x$auc024})),
      colwise(mean)(ldply(metric, function(x) {x$auc0inf})),
      colwise(CI90lo)(ldply(metric, function(x) {x$auc0inf})),
      colwise(CI90hi)(ldply(metric, function(x) {x$auc0inf})),
      colwise(mean)(ldply(metric, function(x) {x$cmax})),
      colwise(CI90lo)(ldply(metric, function(x) {x$cmax})),
      colwise(CI90hi)(ldply(metric, function(x) {x$cmax})),
      colwise(mean)(ldply(metric, function(x) {x$tmax})),
      colwise(CI90lo)(ldply(metric, function(x) {x$tmax})),
      colwise(CI90hi)(ldply(metric, function(x) {x$tmax}))
    )
    names(out.metrics) <- c(
      "mean.auc024", "CI90lo.auc024", "CI90hi.auc024",
      "mean.aucinf", "CI90lo.aucinf", "CI90hi.aucinf",
      "mean.cmax", "CI90lo.cmax", "CI90hi.cmax",
      "mean.tmax", "CI90lo.tmax", "CI90hi.tmax"
    )
    list(
      mean.times = colwise(mean)(ldply(metric, function(x) {x$times})),
      metrics = out.metrics
    )
  })
...
# Determine AUC0-24 of original mean pooled data
  ...
  saveRDS(interv.res, "osu5115_optim.rds")
  saveRDS(out, "osu5115_simandobs.rds")