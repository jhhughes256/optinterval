...
#  So, consider the following dataset, with the following spline regression,
  ...
  K <- c(14, 20)  # knots
  plot(cars)
  lm.mod <- lm(dist ~ ns(speed, knots = c(K)), data = cars)
...
# To determine the number of parameters for optimisation, normally the degree of
# the spline and the number of knots must be considered. These are added
# together to determine the number of components there are to the linear model.

# With natural splines, it is the number of knots + 1
...
# Function for a natural spline with 2 knots
# y = beta1*x[,1] + beta2*x[,2] + beta3*x[,3] + beta0
  ls.spline2k.func <- function(par, x, y) {
    b0 <- par[1]
    b1 <- par[2]
    b2 <- par[3]
    b3 <- par[4]
    err <- y - b1*x[,1] - b2*x[,2] - b3*x[,3] - b0
    sse <- sum(err^2)
  }

  ls.spline2k.res <- optim(
    c(40, 105, 72, 2),  # Initial parameter estimates
    ls.spline2k.func,  # Fitting function
    method = "BFGS",
    x = ns(cars$speed, knots = c(K)), y = cars$dist # Function arg.
  )
...
# into a basis spline matrix, as dictated by the spline we were fitting
  plot(cars)
  ls.spline.u <- ns(u, knots = c(K))
  ls.spline.Y <- ls.spline2k.res$par[2]*ls.spline.u[,1] +
...
# That way the basis matrix will match up with the numbering of the parameters
  flex.u <- ns(u, knots = c(K))
  flex.optres <- list(
    par = c(
      ls.spline2k.res$par[2],
      ls.spline2k.res$par[3],
      ls.spline2k.res$par[4],
      ls.spline2k.res$par[1]
    )
  )
  flex.optres.l <- length(flex.optres$par)
  flex.x <- ns(cars$speed, knots = c(K))

# -   -   -   -   -   -   -   -   -   -   -   -   -   -   -   -   -   -   -   -
# For fitting
# x = ns(cars$speed, knots = c(K)), y = cars$dist
  flex.err <- cars$dist
  ...
  flex.sse != ls.spline2k.res$value
  ...
  test$value != ls.spline2k.res$value
...
# K = the knots used to fit the spline
  predict.spline <- function(x, par, K) {
    ns.mat <- ns(x, knots = c(K))
    out <- double(length(x))
    for (i in 1:length(par)) {
      if (i != length(par)) {
        out <- out + par[i]*ns.mat[, i]
      } else if (i == length(par)) {
        out <- out + par[i]
      }
    }
...
# another parameter
  ...
  mle.test.Y <- predict.spline(u, mle.test.res$par[-length(mle.test.res$par)], K)
  plot(cars)
...
# This isn't perfect though if you don't have good initial estimates
  ...
  mle.test.res2 <- optim(
    c(50, 100, 60, 1),  # Initial parameter estimates
    mle.spline,  # Fitting function
    method = "BFGS",
    x = ns(cars$speed, knots = c(K)), y = cars$dist # Function arg.
  )
...
# Find our initial estimates using QR decomposition via lm()
  pk.modlm <- lm(conc ~ ns(time, knots = c(K)), data = onecomp)
...
# Supress warnings due to NaN being produced by estimating sigma of < 0
  suppressWarnings(
    pk.optres <- optim(
      ...
      mle.spline,  # Fitting function
      method = "BFGS",
      x = ns(onecomp$time, knots = c(K)), y = onecomp$conc # Function arg.
    )
  )
  ...
  pk.mle.Y <- predict.spline(u, pk.optres$par[-length(pk.optres$par)], K)
  plot(onecomp)
