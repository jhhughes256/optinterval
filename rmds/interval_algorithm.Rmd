...
This is achieved using integration by parts and the proof can be found
[here](http://www.math.ucsd.edu/~ebender/20B/77_Trap.pdf).
...
```{r final.output, echo = F}
  ...
  plotdata <- data.frame(
    time = data1$time,
    cobs = data1$conc,
    pred = pred.sumexp(temp2$par, data[, 1])
  )
  ylim <- c(0, 1.1*max(plotdata$cobs))
  xlim <- c(0, 24)
  plotobj <- NULL
