...
The function that is used to optimise the sum of exponential uses maximum
likelihood estimation. Final parameter from optim is removed; this is sigma.
Sigma is a parameter so that it is minimised, but has boundaries so that it 
doesn't reach zero.
...
But the first step of the algorithm is to determine the sum of exponentials that 
...
# Fit sum of exponentials to curve for different numbers of exponentials
  ...
  # Collate data and ready final objects
  ...
  # Loop for maximum number of exponentials
    ...
    # For the first two loops lm.par is used to determine initial parameters
      ...
      browser()
    # For each number of exponentials optimise the sum of exponential parameters
    # slopes have upper bound of ~0 as all curves will be exponential decay
    # sigma has lower bound of 0.0001, as sigma != 0
      optres <- optim(
        init.par,  # Sigma parameter added to init.par for optimisation
    ...
    # record values, while removing sigma
...
initial parameters that are used. This is certainly an area that can be 
improved, however improvements are not always easily noticable.

This function produces the following output when used on oral drug data:
