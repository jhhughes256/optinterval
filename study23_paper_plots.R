...
# True vs. Pred
  ...
  truevpred.plot <- function(data, metric, n.col) {
    ...
    logscaleticks <- 10^c(
      seq(from = floor(log10(signif(minval, 1))), to = ceiling(log10(signif(maxval, 1))), by = 1)
    )
    logscaleticks2 <- sort(c(logscaleticks, logscaleticks*3))
    ...
    if (metric == "tmax") {
      ...
      p0 <- p0 + geom_line(aes(x = ref, y = test), data = bioqloline,
        colour = "green4", linetype = "dashed")
      p0 <- p0 + scale_x_log10("True tmax", lim = c(minval, maxval), breaks = logscaleticks2)
      p0 <- p0 + scale_y_log10("Predicted tmax", lim = c(minval, maxval), breaks = logscaleticks2)
    } else {
      ...
      p0 <- p0 + geom_line(aes(x = ref, y = test), data = bioqloline2,
        colour = "green4", linetype = "dashed")
      if (metric == "cmax") {
        p0 <- p0 + scale_x_log10("True Cmax", lim = c(minval, maxval), breaks = logscaleticks)
        p0 <- p0 + scale_y_log10("Predicted Cmax", lim = c(minval, maxval), breaks = logscaleticks)
      } else {
        p0 <- p0 + scale_x_log10("True AUC", lim = c(minval, maxval), breaks = logscaleticks)
        p0 <- p0 + scale_y_log10("Predicted AUC", lim = c(minval, maxval), breaks = logscaleticks)
      }
    }
  ...
  p1 <- truevpred.plot("d3a", "aucinf", 2)
  p2 <- truevpred.plot("d3a", "cmax", 2)
  p3 <- truevpred.plot("d3a", "tmax", 2)

  p123 <- plot_grid(p1, p2, p3, nrow = 3,
  ...
  p4 <- NULL
  p4 <- ggplot(plotaucinf, aes(x = typef, y = prop))
  p4 <- p4 + geom_hline(yintercept = c(0.9, 1.1), color = "green4", linetype = "dashed")
  ...
  p4 <- p4 + xlab("\nMethod")
  p4 <- p4 + scale_y_continuous("Method/Reference AUC Ratio\n", lim = c(0.1, 1.6))
  p4 <- p4 + facet_wrap(~dataf, nrow = 1)
  p4
