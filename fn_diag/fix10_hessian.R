# Make decisions around optimisation confidence using standard errors
# And more specifically the hessian matrix provided by optim
# -----------------------------------------------------------------------------
# Set up libraries
  library(GA)
# -----------------------------------------------------------------------------
...
# Updated optim.sumexp (using browser)
  optim.sumexp <- function(data, oral = F, nexp = 3) {
    ...
    opt.par <- list(NULL)  # final parameters
    opt.val <- list(NULL)  # objective function value
    opt.gra <- list(NULL)  # gradient
    opt.con <- list(NULL)  # convergence status
    opt.mes <- list(NULL)  # convergence message
    opt.sep <- list(NULL)  # standard error (%)
    lm.sub <- which(y == max(y))[1]:length(y)
    lmres <- lm(log(y[lm.sub]) ~ x[lm.sub])
    lmres.par <- unname(lmres$coefficients)
    for (i in 1:nexp) {
      if (i == 1 & !oral) {
        optres <- list(
          par = c(lmres.par[2], lmres.par[1]),
          value = mle.sumexp(unname(c(lmres.par[2], lmres.par[1])), x, y, 0.01),
          counts = NULL, convergence = 0, message = NULL
        )
        se.par <- sqrt(diag(vcov(lmres)))
        sepercent.par <- unname(rev(se.par/abs(lmres.par)*100))
      } else {
        flag <- 0
        repeat {
          gares <- ga("real-valued",
            mle.sumexp, x = x, y = y, ga = T, sigma = 0.01,
            min = c(rep(lmres.par[2]*50, i + oral), rep(lmres.par[1]-2, i)),
            max = c(rep(lmres.par[2]/50, i + oral), rep(lmres.par[1]+2, i)),
          ...
          se.par <- suppressWarnings(sqrt(diag(solve(optres$hessian))))
          sepercent.par <- se.par/abs(optres$par)*100
          if (optres$convergence != 0 & any(is.nan(se.par)) & flag != 10) {
            flag <- flag + 1
          } else {
            break
          }
      ...
      opt.mes[[i]] <- ifelse(is.null(optres$message),
        "NULL", optres$message)
      opt.sep[[i]] <- sepercent.par
    }
    res <- list(par = opt.par, value = opt.val, counts = opt.gra,
      convergence = opt.con, message = opt.mes, se = opt.sep)
    res
  }
