...
# Two Compartment Kinetics
  ...
  d2b.all <- pred.d2b(time.samp, d2b.p)
  ...
  eps1 <- matrix(
    rnorm(n = length(d2b.t)*niter, mean = 0, sd = err.sig),
    nrow = length(d2b.t), ncol = niter
  )
  if (sdev <= 2) {
    d2b <- d2b.sub*(1 + eps1)
  } else {
    eps2 <- rnorm(n = length(d2b.t), mean = 0, sd = tail(d2b.sub, 1)*err.sig)
...
# Three Compartment Kinetics
  ...
  d3b.all <- pred.d3b(time.samp, d3b.p)
  ...
  if (sdev == 4) err.sig <- 0.1
  eps1 <- matrix(
    rnorm(n = length(d3b.t)*niter, mean = 0, sd = err.sig),
    nrow = length(d3b.t), ncol = niter
  )
  if (sdev <= 2) {
    d3b <- d3b.sub*(1 + eps1)
  } else {
    eps2 <- rnorm(n = length(d3b.t), mean = 0, sd = tail(d3b.sub, 1)*err.sig)
...
# One Compartment Kinetics w/ Absorption
  ...
  d1a.p <- c(-0.2, -0.4, 4)
  d1a.all <- pred.d1a(time.samp, d1a.p)
  ...
  if (sdev == 4) err.sig <- 0.1
  eps1 <- matrix(
    rnorm(n = length(d1a.t)*niter, mean = 0, sd = err.sig),
    nrow = length(d1a.t), ncol = niter
  )
  if (sdev <= 2) {
    d1a <- d1a.sub*(1 + eps1)
  } else {
    eps2 <- rnorm(n = length(d1a.t), mean = 0, sd = tail(d1a.sub, 1)*err.sig)
...
# Two Compartment Kinetics w/ Absorption
  ...
  d2a.all <- pred.d2a(time.samp, d2a.p)
  ...
  if (sdev == 4) err.sig <- 0.1
  eps1 <- matrix(
    rnorm(n = length(d2a.t)*niter, mean = 0, sd = err.sig),
    nrow = length(d2a.t), ncol = niter
  )
  if (sdev <= 2) {
    d2a <- d2a.sub*(1 + eps1)
  } else {
    eps2 <- rnorm(n = length(d2a.t), mean = 0, sd = tail(d2a.sub, 1)*err.sig)
...
# Three Compartment Kinetics w/ Absorption
  ...
  d3a.all <- pred.d3a(time.samp, d3a.p)
  ...
  if (sdev == 4) err.sig <- 0.1
  eps1 <- matrix(
    rnorm(n = length(d3a.t)*niter, mean = 0, sd = err.sig),
    nrow = length(d3a.t), ncol = niter
  )
  if (sdev <= 2) {
    d3a <- d3a.sub*(1 + eps1)
  } else {
    eps2 <- rnorm(n = length(d3a.t), mean = 0, sd = tail(d3a.sub, 1)*err.sig)
